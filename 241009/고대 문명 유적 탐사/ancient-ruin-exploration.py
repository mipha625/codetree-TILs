...
while K > 0: #턴이 0보다 클 때까지
    ...
    for d in range(90, 271, 90):
        for i in range(3):
            for j in range(3):
                ...
                rotate_li.append(rotate.rotate_degree(arr1, i, j, d))
                
    for i in range(27):
        total, temp = search.find([items[:] for items in rotate_li[i]])
    ...
    while True:
        ...
        #유물 연쇄 획득 확인
        max_total, max_temp = search.find([items[:] for items in arr])
        result += max_total
